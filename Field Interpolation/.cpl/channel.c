...
/*!(ARRAY(0..1000000) OF REAL U
ARRAY(0..1000000) OF REAL VV
ARRAY(0..1000000) OF REAL W
FILE u_dat;		u_dat = OPEN("u.dat")
FILE w_dat;		w_dat = OPEN("w.dat")
FILE v_dat;		v_dat = OPEN("v.dat")
...
LOOP FOR k=LO2(V) TO HI2(V)
	LOOP FOR i=LO(V) TO HI(V)
		LOOP FOR j=LO3(V) TO HI3(V)
			V.u.REAL(i,k,j)=U[counter]
			V.v.REAL(i,k,j)=VV[counter]
			V.w.REAL(i,k,j)=W[counter]
			INC counter
			V.u.IMAG(i,k,j)=U[counter]
			V.v.IMAG(i,k,j)=VV[counter]
			V.w.IMAG(i,k,j)=W[counter]
			INC counter
...
REPEAT LOOP  !)*/
	...
	static struct timeval tm1, tres; 
...
forward:; while( time_ <t_max_-deltat_/2.){
  ...
  if( reread_ ){ dnsdata_1read_initial_data(); reread_=0;};
	...
	static struct timeval ttot; 
	gettimeofday(&tm2, NULL);
	timersub(&tm2, &tm1, &ttot);
	printf("Time elapsed: %ld.%06ld\n", (long int)ttot.tv_sec, (long int)ttot.tv_usec);
...
LOOP FOR iy = miny TO maxy
	WRITE "iy="iy
     LOOP FOR ALL ix,iz WITH V(ix,iz,iy)
		WRITE "("ix","iz","iy") " u, v
     REPEAT LOOP
	WRITE
	WRITE
